...
def smooth_one_sided_causal_gate(delta_t, tau=1.0):
    ...
    return jnp.where(delta_t <= 0.0, 0.0, jnp.where(delta_t >= tau, 1.0, smooth))
...
def predict_mimo_causal_init(
    ...
    branch_width = branch_ranges[0][1] - branch_ranges[0][0]
    control_stack = x_inputs.reshape(
        x_inputs.shape[0], len(branch_ranges), branch_width
    )

    if encoder_time_features is None:
        encoder_time_features = (jnp.arange(len(branch_ranges), dtype=x_inputs.dtype)[None, :, None] * sample_time)
...
def load_bundle(path: str | Path) -> DeepONetBundle:
    with np.load(path) as data:
        ...
        return DeepONetBundle(
            ...
            sample_time=(float(data["sample_time"]) if "sample_time" in data.files else 0.05),
        )
...
def predict_states_u_jacobian(bundle: DeepONetBundle,  x0: jnp.ndarray, u_seq: jnp.ndarray) -> jnp.ndarray:
    x0 = jnp.asarray(x0, dtype=jnp.float32)
    u_seq = jnp.asarray(u_seq, dtype=jnp.float32)
