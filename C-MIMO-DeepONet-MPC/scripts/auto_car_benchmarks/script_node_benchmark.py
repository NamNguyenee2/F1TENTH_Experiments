"""Benchmark residual NODE terminal inference and Jacobians on CPU and GPU."""
...
from baseline.auto_car_node.script_node import load_bundle, node_step  # noqa: E402
...
def terminal_state(params, x0, u_seq):
    ...
    x_terminal, _ = jax.lax.scan(scan_step, x0, u_seq)
    return x_terminal


def terminal_full_u_jacobian(params, x0, u_seq):
    """Compute d x[H|k] / d u[0:H-1|k], shape (n_x, H, n_u)."""
    return jax.jacfwd(
        lambda controls: terminal_state(params, x0, controls)
    )(u_seq)
...
def benchmark_device(device, params_host, x0_host, u_host):
    """Compile and benchmark all NODE operations on one explicit device."""
    ...
    functions = {
        "inference": jax.jit(terminal_state),
        "full_jacobian": jax.jit(terminal_full_u_jacobian),
    }
...
def main():
    ...
    for _, device in devices:
        benchmark_device(device, params_host, x0_host, u_host)
